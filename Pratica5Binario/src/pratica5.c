...
SemaphoreHandle_t semaphoneBinary;
QueueHandle_t queue;
...
void button(void *pvParameters) {
    while (1) {
        if ((xSemaphoreTake(semaphoneBinary, portMAX_DELAY) == pdTRUE) && click) {
            ...
            xSemaphoreGive(semaphoneBinary);
...
void semaphore(void *pvParameteres) {
	while (1) {
		if (xSemaphoreTake(semaphoneBinary, portMAX_DELAY) == pdTRUE) {
...
void button_isr(uint gpio, uint32_t events) {
    ...
    if (current_time - last_interrupt_time > 300) { // 300 ms debounce
        ...
        xSemaphoreGiveFromISR(semaphoneBinary, &higherPriorityTaskWoken);
...
int main() {
    ...
    semaphoneBinary = xSemaphoreCreateBinary(); 
    queue = xQueueCreate(10, sizeof(led_command_t));

    xTaskCreate(semaphore, "SemaphoreTask", 256, NULL, 1, NULL);
